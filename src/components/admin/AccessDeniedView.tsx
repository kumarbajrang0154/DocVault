...
export function AccessDeniedView({ userEmail }: AccessDeniedViewProps) {
  const handleSignOut = () => {
    ...
    <div className="flex min-h-[80vh] items-center justify-center p-4 sm:p-6 lg:p-8">
      ...
      <div className="w-full max-w-md overflow-hidden rounded-3xl border border-rose-500/20 bg-zinc-900/80 p-8 sm:p-10 shadow-2xl backdrop-blur-2xl text-center">
        ...
        <p className="mt-3 text-sm text-zinc-400 leading-relaxed">
          The signed-in Google account <span className="font-semibold text-zinc-200">{userEmail || 'associated with your session'}</span> is not authorized to access the Mood Admin Portal.
        </p>
        ...
        <div className="mt-8 flex flex-col gap-3">
          <button
            ...
            className="inline-flex h-11 items-center justify-center gap-2 rounded-full bg-rose-500 px-6 text-sm font-bold text-white transition-all hover:bg-rose-600 active:scale-98"
          >
          ...
          <Link
            href="/"
            className="inline-flex h-11 items-center justify-center gap-2 rounded-full border border-white/10 bg-white/5 px-6 text-sm font-semibold text-zinc-300 transition-all hover:bg-white/10 hover:text-white"
          >
            <ArrowLeft className="h-4 w-4" />
            <span>Return to Mood Landing Page</span>
          </Link>
        </div>
